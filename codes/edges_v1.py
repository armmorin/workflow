...
def find_nearest_neighbors(structure, atom_index):
    ...
    Returns:
    list of tuples: Each tuple contains (neighbor_index, distance).
    """
    ...
    with warnings.catch_warnings():
        ...
        neighbor_info = [(neighbor['site_index'], neighbor['weight']) for neighbor in neighbors]
...
def analyze_coordination(structure, atom_indices, force_coordination_number=None):
    """
    ...
    force_coordination_number (int, optional): Exact number of nearest neighbors to use. Defaults to None.
    ...
    for atom_index in atom_indices:
        ...
        if force_coordination_number is not None:
            print(f"\nAtom {atom_index}: Original nearest neighbors count = {original_count}")
            if original_count > force_coordination_number:
                # Sort neighbors based on distance (assuming 'weight' is distance)
                neighbors_sorted = sorted(neighbors, key=lambda x: x[1])
                selected_neighbors = neighbors_sorted[:force_coordination_number]
                discarded_neighbors = neighbors_sorted[force_coordination_number:]
                coordination_environments[atom_index] = [n[0] for n in selected_neighbors]
                print(f"force_coordination_number is set to {force_coordination_number}.")
                print(f"Selected {len(selected_neighbors)} nearest neighbors based on shortest distances.")
                print(f"Discarded {len(discarded_neighbors)} nearest neighbors.")
            elif original_count < force_coordination_number:
                print(f"Warning: Atom {atom_index} has only {original_count} neighbors, which is less than the forced coordination number {force_coordination_number}. Using all available neighbors.")
                coordination_environments[atom_index] = [n[0] for n in neighbors]
            else:
                coordination_environments[atom_index] = [n[0] for n in neighbors]
                print(f"Atom {atom_index} has exactly {force_coordination_number} nearest neighbors. No changes made.")
        else:
            coordination_environments[atom_index] = [n[0] for n in neighbors]
...
def calculate_diagonals(structure, atom_indices):
    ...
    for atom_index in atom_indices:
        ...
        if len(neighbors) == 6:  # Ensure it's an octahedron
            ...
            neighbor_positions = [adjust_position_with_pbc(structure[neighbor].position, central_position, cell) for neighbor in [n[0] for n in neighbors]]
...
def analyze_coordination_and_edges(structure, atom_index, alignment_vector, force_coordination_number=None):
    """
    ...
    """
    # Retrieve coordination environments with optional forcing
    coordination_env = analyze_coordination(structure, [atom_index], force_coordination_number)
    neighbors = coordination_env.get(atom_index, [])
...
def calculate_angle_between_edges(structure1, index1, vector1, structure2, index2, vector2, 
                                  force_coordination_number1=None, force_coordination_number2=None):
    """
    ...
    force_coordination_number1 (int, optional): Exact number of nearest neighbors to use for structure1. Defaults to None.
    force_coordination_number2 (int, optional): Exact number of nearest neighbors to use for structure2. Defaults to None.

    Returns:
    tuple: (angle, edge1, edge2) where angle is the minimal angle (in degrees) between the two most aligned edges,
    ...
    neighbors1, edges1 = analyze_coordination_and_edges(structure1, index1, alignment_vector1, force_coordination_number1)
    if not edges1:
        print(f"No edges found for atom {index1} in structure1.")
        return None, None, None
    edge1 = edges1[0]  # Most aligned edge for structure1
    #print(f"\nStructure1 - Atom {index1}:")
    #print(f"Most aligned edge to atom {edge1[0]}: length = {edge1[1]:.3f}, angle = {edge1[2]:.2f} degrees, alignment score = {edge1[3]:.2f}")
    
    # Process the second structure
    alignment_vector2 = interpret_vector_input(structure2, vector2)
    neighbors2, edges2 = analyze_coordination_and_edges(structure2, index2, alignment_vector2, force_coordination_number2)
    if not edges2:
        print(f"No edges found for atom {index2} in structure2.")
        return None, None, None
    edge2 = edges2[0]  # Most aligned edge for structure2
    #print(f"\nStructure2 - Atom {index2}:")
    #print(f"Most aligned edge to atom {edge2[0]}: length = {edge2[1]:.3f}, angle = {edge2[2]:.2f} degrees, alignment score = {edge2[3]:.2f}")
    ...
    #print("\nStructure1 - Central Atom, Neighbor Atom, Connecting Vector")
    #print(central_position1, neighbor_position1, edge_vector1)
    ...
    #print("\nStructure2 - Central Atom, Neighbor Atom, Connecting Vector")
    #print(central_position2, neighbor_position2, edge_vector2)
    
    # Calculate angle between edge vectors
    angle = calculate_angle(edge_vector1, edge_vector2)
    
    # Apply minimal angle logic only for the final angle
    minimal_angle = min(angle, 180 - angle)
    #print(f"\nCalculated minimal angle between most aligned edges: {minimal_angle:.2f} degrees")
    
    #print(neighbors1)
    
    #print(neighbors2)
    
    return minimal_angle, edges1, edges2

def main(filename, metal_index, alignment_vector_input, 
         force_coordination_number1=None, force_coordination_number2=None, 
         print_edges=False):
    """
    ...
    neighbors, edges = analyze_coordination_and_edges(structure, metal_index, alignment_vector, force_coordination_number1)
...
if __name__ == "__main__":
    ...
    poscar_file = 'POSCAR_ts'
    metal_index_poscar = 44
    # Example with force_coordination_number enabled
    # result_poscar = main(poscar_file, metal_index_poscar, 'c', force_coordination_number1=6, force_coordination_number2=6, print_edges=True)
    ...
    structure1 = read_structure(argv[1])#"POSCAR_Ca7Nb4MoO20_p3_pristine")  #read_structure('POSCAR_r')
    structure2 = read_structure(argv[2])#"POSCAR_Ca7Nb4MoO20_p3_tstate") #read_structure('POSCAR_ts')
    index1 = int(argv[3]) #7 # Example atom index for structure1
    index2 = int(argv[4]) #28  # Example atom index for structure2
    vector1 = [1,1,-1] #'c' # Alignment vector for structure1
    vector2 = [1,1,-1] #'c' # Alignment vector for structure2

    # Example with force_coordination_number enabled for both structures
    angle, edge1, edge2 = calculate_angle_between_edges(
        structure1, index1, vector1, 
        structure2, index2, vector2, 
        force_coordination_number1=4, 
        force_coordination_number2=4
    )

    if angle is not None:
        print(f"\nAngle between most aligned edges: {angle:.2f} degrees")
        print(f"Edge 1: to atom {edge1[0]}, length = {edge1[1]:.3f}, angle = {edge1[2]:.2f} degrees, alignment score = {edge1[3]:.2f}")
        print(f"Edge 2: to atom {edge2[0]}, length = {edge2[1]:.3f}, angle = {edge2[2]:.2f} degrees, alignment score = {edge2[3]:.2f}")